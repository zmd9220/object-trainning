...
public:
	...
	void DFS(int start)
	{
		if (v[start]->get_visited() == true)
			return;
		v[start]->set_visited();
		for (int i = 0; i < max_sz; i++) //인접 행렬 기반 - 모든 행에 대해 확인
		{
			if (v[i]->get_visited() == false) // 다음 정점이 visit 되지 않았으면 발견한 상태(맨 처음 발견)
			{
				...
				if (matrix[start][i]->get_discovery()) continue; //(이미 발견된 상태면서 이미 그 루트가 누군가에게 발견된 상태 = 맨처음 발견이 아님)-discovered edge 종료 후 다음 위치로
				else {
					matrix[start][i]->set_explore();
					matrix[start][i]->set_back(); // 누군가에게 발견된 상태는 아니지만
				}
			}
		}
	}
